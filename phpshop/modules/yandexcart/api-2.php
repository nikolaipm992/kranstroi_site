    ...
    case('/cart'):
        ...
        if ($option['model'] === 'DBS') {
            ...
            foreach ($storeDeliveries as $delivery) {
                if ((int) $delivery['yandex_region_id_2'] === (int) $data['cart']['delivery']['region']['id'] or (int) $delivery['yandex_region_id_2'] === 0) {
                    $deliveries[] = $delivery;
                }
            }
            ...
            if (count($deliveries) === 0) {
                foreach ($storeDeliveries as $delivery) {
                    if (in_array($delivery['yandex_region_id_2'], $regionIds) or (int) $delivery['yandex_region_id_3'] === 0) {
                        $deliveries[] = $delivery;
                    }
                }
            }
            ...
            foreach ($deliveries as $delivery) {
                ...
                if ($type === 'PICKUP') {
                    $outlets = array_unique(unserialize($delivery['yandex_delivery_points_2']));
                    if (!is_array($outlets)) {
                        $outlets = [];
                    }
    ...
    case "/order/status":
        ...
        if ($data['order']['status'] === 'PROCESSING') {
            ...
            $row = (new PHPShopOrm($GLOBALS['SysValue']['base']['orders']))
                    ->getOne(['*'], ['yandex_order_id_2' => sprintf("='%s'", $data['order']['id'])]);
