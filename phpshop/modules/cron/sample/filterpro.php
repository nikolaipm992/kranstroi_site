...
class CacheFilter {
    ...
    private function countProducts($valueId, $sortId, $categoryId) {
        $orm = new PHPShopOrm();
        $orm->debug = false;
        $result = $orm->query(sprintf('select COUNT("id") as count from `phpshop_products` where (`category`="%s" or `dop_cat` REGEXP "#%s#") and vendor REGEXP "i%s-%si" and enabled="1" and parent_enabled="0"', $categoryId, $categoryId, $sortId, $valueId));
