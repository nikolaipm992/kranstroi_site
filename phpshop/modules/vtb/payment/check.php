<?php
/**
 * Обработчик оповещения о платеже ВТБ
...
include_once($_classPath . "modules/vtb/hook/mod_option.hook.php");
...
$PHPShopModules->checkInstall('vtb');
...
class Payment extends PHPShopPaymentResult {
    ...
    function updateorder() {
        ...
        // Не доверяем полученному уведомлению и делаем повторный запрос в ВТБ.
    ...
    private function isOrderPaid($orderNumber, $merchantId) {
        ...
        if (isset($r['ErrorCode'])) {
            ...
            $PHPShopVtbArray->log($r, $orderNumber, 'Ошибка проведения платежа', 'Запрос состояния заказа');
        } elseif ($r['orderStatus'] != 2) {
            $code_description = PHPShopString::utf8_win1251($r['actionCodeDescription']);
            $PHPShopVtbArray->log($r, $orderNumber, $code_description, 'Запрос состояния заказа');
        } else {
            $PHPShopVtbArray->log($r, $orderNumber, 'Платеж проведен', 'Запрос состояния заказа');
        }
