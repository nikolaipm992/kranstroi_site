...
function actionStart() {
    ...
    $Tab1 = $PHPShopGUI->setCollapse('Основное', $PHPShopGUI->setField('Название', $PHPShopGUI->setInputText('', 'name_new', $data['name'])) .
            ...
            $PHPShopGUI->setField('Формула',
                    $PHPShopGUI->setRadio("discount_tip_new", 0, "При заказе N количества товара А, получи скидку Б на товар А", $data['discount_tip']) . '<br>'.
                    $PHPShopGUI->setRadio("discount_tip_new", 1, "При заказе N количества товара А, цена для товара А берется из поля Цена Б", $data['discount_tip'])).
