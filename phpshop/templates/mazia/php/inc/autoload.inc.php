...
if ($GLOBALS['SysValue']['template_theme']['user'] == 'true' or ! empty($GLOBALS['SysValue']['other']['skinSelect'])) {
    ...
    $PHPShopGUI->addJSFiles($GLOBALS['SysValue']['dir']['templates'] . chr(47) . $_SESSION['skin'] . '/js/editor.js');
    ...
    // Шапка
    $css_edit_theme .= '<p><br></p>'.$PHPShopGUI->setCollapse('Шапка', $PHPShopGUI->setLink('#', $PHPShopGUI->setImage('images/editor/icon8.svg', 65, 55, null, 3, null, null, null, @$h_active1), null, null, 'Шапка 1', 'setBlock', 'mazia_h=1') .
            $PHPShopGUI->setLink('#', $PHPShopGUI->setImage('images/editor/icon7.svg', 65, 55, null, 3, null, null, null, @$h_active2), null, null, 'Шапка 2', 'setBlock', 'mazia_h=2') .
            $PHPShopGUI->setLink('#', $PHPShopGUI->setImage('images/editor/icon6.svg', 65, 55, null, 3, null, null, null, @$h_active3), null, null, 'Шапка 2', 'setBlock', 'mazia_h=3')
            , 'in', false, false);

    $PHPShopGUI->nav_style = 'nav-tabs';
    $css_edit .= $css_edit_theme;

    // Сохранить
    if (!empty($_SESSION['logPHPSHOP'])) {
        $css_edit .= '<br>' . $PHPShopGUI->setButton('Сохранить', 'floppy-disk', 'btn-xs btn-secondary saveTheme');
        $admin_edit .= $PHPShopGUI->setButton('Управлять', 'cog', 'btn-xs btn-secondary openAdminModal');
        ...
        $css_edit .= $PHPShopGUI->setButton('Отладка', 'picture', 'btn-xs btn-secondary setDebug' . $debug_active);
    }
    ...
    if (is_array($GLOBALS['SysValue']['connect_select'])) {
        ...
        $GLOBALS['SysValue']['other']['skinSelect'] .= PHPShopParser::file($GLOBALS['SysValue']['dir']['templates'] . chr(47) . $_SESSION['skin'] . chr(47) . $GLOBALS['SysValue']['templates']['right_menu'], true, true, false);
    }
